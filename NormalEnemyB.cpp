...
void NormalEnemyB::Init()
{
	for (size_t i = 0; i < state_obj_.TexSize_; i++) {
		state_obj_.Model_[i] = Shapes::CreateSquare({0, 0}, {128.0f, 128.0f}, "dogomu_face.png", {128.0f, 64.0f}, {0.5f, 0.5f}, {128.0f * (float)i, 0.0f}, {128.0f, 128.0f});
	}
...
void NormalEnemyB::Jump()
{
	...
	m_JumpInter += 1.f / 60.f;
	_status.Pos.y = GroundY + (1.0f - pow(1.0f - sin(pi * m_JumpInter), Distortion)) * Height;

	m_JumpInter = std::clamp(m_JumpInter, 0.f, 1.f);
}
