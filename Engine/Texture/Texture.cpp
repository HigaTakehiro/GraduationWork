...
DirectXSetting* Texture::Dxcomn = nullptr;
...
bool Texture::StaticInitialize(DirectXSetting*dxc,int window_width, int window_height)
{
	Dxcomn = dxc;
	device =Dxcomn->GetDev();
	cmdList =Dxcomn->GetCmdList();
...
bool Texture::InitializeGraphicsPipeline()
{
	...
	result = D3DCompileFromFile(
		L"Engine/Resources/shaders/Object2d/TextureVS.hlsl", // シェーダファイル名
		nullptr,
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Engine/Resources/shaders/Object2d/TexturePS.hlsl", // シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
void Texture::Update(Camera* camera)
{
	//anchorpoint = { 0,0 };
	HRESULT result;
	...
	constMap->matbillbord = camera->GetMatBillboard();
...
void Texture::Update(XMMATRIX matw, Camera* camera)
{
	//anchorpoint = { 0,0 };
	HRESULT result;
	XMMATRIX matScale, matRot, matTrans;
